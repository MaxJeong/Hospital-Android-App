...
public class HomeScreen extends Activity implements OnItemClickListener, OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		while (listofpatients.hasNext()){
			...
			HCNList.add(currentRecord.getValue().getName()+" "+ currentRecord.getValue().getDob()+" "+currentRecord.getKey());
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
		...
		patientViewIntent.putExtra("PatientHCNKey", PatientArray[position].getHcn());
	...
	public void onClick(View v) {
		...
		case R.id.BTSave:
			try {
				LoginAuthentication.pmanager.writeToFile(HospitalRecord.getListOfPatients().values().iterator());
				LoginAuthentication.vsmanager.writeToFile(HospitalRecord.getListOfPatients().values().iterator());
				LoginAuthentication.smanager.writeToFile(HospitalRecord.getListOfPatients().values().iterator());
				LoginAuthentication.prmanager.writeToFile(HospitalRecord.getListOfPatients().values().iterator());
	...
	public void onRadioButtonClicked(View view){
		...
		case R.id.RBAll:
			if (checked){
				sortSelected = 0;
				updateList(HospitalRecord.getListOfPatients().entrySet().iterator());
			}
			break;
		case R.id.RBTime:
			if (checked){
				sortSelected = 1;
				updateList(HospitalRecord.getListOfEarliestArrivingPatients().entrySet().iterator());
			}
			break;
		case R.id.RBUrgency:
			if (checked){
				sortSelected = 2;
				updateList(HospitalRecord.getListOfMostUrgentPatients().entrySet().iterator());
			}
			break;
		}
	}
	
	public void updateList(Iterator<Map.Entry<String, Patient>> listofpatients){
